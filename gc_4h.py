...
MAX_MINUTES_AFTER_BOX_CLOSE: Optional[int] = None
...
SWEEP_TOP_N = 10
SWEEP_RR_VALUES = [4.0]
SWEEP_MAX_MINUTES_AFTER_BOX_CLOSE: Sequence[Optional[int]] = [None]
SWEEP_MIN_BREAKOUT_PCTS: Sequence[Optional[float]] = [None, 0.085]
SWEEP_MIN_BREAKOUT_WICK_PCTS: Sequence[Optional[float]] = [None,0.2]
SWEEP_MAX_BREAKOUT_PCTS: Sequence[Optional[float]] = [None,0.7]
SWEEP_MAX_REENTRY_MINUTES: Sequence[Optional[int]] = [None,105]
SWEEP_MIN_BOX_SIZES: Sequence[Optional[float]] = [3.0]
SWEEP_MAX_BOX_SIZES: Sequence[Optional[float]] = [None]
SWEEP_MIN_RETEST_SIZE_PCTS: Sequence[Optional[float]] = [None,0.12]
SWEEP_MAX_RETEST_SIZE_PCTS: Sequence[Optional[float]] = [None]
SWEEP_MIN_RETEST_BODY_INSIDE_PCTS: Sequence[Optional[float]] = [None]
SWEEP_MIN_TICKS_TO_STOP: Sequence[Optional[float]] = [None,10]
SWEEP_MAX_TICKS_TO_STOP: Sequence[Optional[float]] = [None,70]
        ...
        for bar in day.bars:
            ...
            if pending_breakout is not None:
                ...
                if pending_breakout["direction"] == "above":
                    if (range_low <= bar.open <= range_high) and bar.close > range_high:
                        pending_breakout["stop"] = max(pending_breakout["stop"], bar.high)
                    elif (range_low <= bar.open <= range_high) and bar.close < range_low:
                        if (
                            range_size == 0
                    ...
                    elif (range_low <= bar.open <= range_high) and bar.close > range_high:
                        if (
                            range_size == 0
                            or min_breakout_pct is None
