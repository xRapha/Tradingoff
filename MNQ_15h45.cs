...
{
    // Constructeur (optionnel mais propre)
    public SBA_LondonBreak_EMA50_6E_Live()
    {
        this.InstanceName = "MNQ_15h45";
    }
    ...
    protected override void OnCreated()
    {
        ...
        // Si jamais la plateforme ne l’a pas déjà remplie
    ...
    [InputParameter("Order Tag", 70)] public string Tag = "MNQ_15h45";
    ...
    {
        const string targetSymbolId = "MNQ@CME";

        if (Symbol == null || Symbol.Id != targetSymbolId)
        {
            Symbol = Core.Instance.Symbols
                .FirstOrDefault(s => s.Id == targetSymbolId);

            if (Symbol == null)
            {
                Log($"Symbole introuvable : {targetSymbolId}", StrategyLoggingLevel.Error);
                return;
            }
        }

        const string targetAccountName = "LTF050-W8LX83K4-PRO001 LT-46UMR70B";

        // Si ce n’est pas déjà le bon compte, on le force par le Name
        if (Account == null ||
            !string.Equals(Account.Name, targetAccountName, StringComparison.OrdinalIgnoreCase))
        {
            Account = Core.Instance.Accounts
                .FirstOrDefault(a => string.Equals(a.Name, targetAccountName, StringComparison.OrdinalIgnoreCase));

            if (Account == null)
            {
                Log($"Compte introuvable (par Name) : {targetAccountName}", StrategyLoggingLevel.Error);

                // Debug : log tous les comptes pour voir Id / Name
                foreach (var acc in Core.Instance.Accounts)
                    Log($"[ACC] Id='{acc.Id}'  Name='{acc.Name}'");

                return;
            }
        }
        // --- RESET ÉTAT AU DÉMARRAGE MANUEL ---
        breakFound = false;
        tradePlaced = false;
        beArmed = false;
        beActivated = false;
        ...
        liveO = liveH = liveL = liveC = 0.0;
        liveV = 0;
        live1mAtClose.Clear();
        aggTF.Clear();
        validatedBuckets.Clear();

        Log("[INIT] MNQ_15h45 reset complet (nouveau run manuel)");
    ...
    private void ProcessMinuteBar(DateTime barOpenUtc, double open, double high, double low, double close, bool allowTrading, bool fromHistory)
    {
        ...
        foreach (var bucket in readyBuckets)
        {
            ...
            if (bucket.Samples < m)
            {
                // Affiche les valeurs O/H/L/C déjà agrégées pour diagnostic
                Log($"[WARN] {m}m bucket incomplet {bucket.OpenUtc:HH:mm}->{bucket.CloseUtc:HH:mm} " +
                    $"({bucket.Samples}/{m} min) — tentative via officiel. " +
                    $"O={bucket.O:F5} H={bucket.H:F5} L={bucket.L:F5} C={bucket.C:F5}");
                // ⚠️ Ne pas faire continue ici : on laisse GetOfficialTFBarAggregated tenter la reconstitution
            }
            ...
            if (!IsInRange(bucketCloseLon.TimeOfDay, entryStart, entryCut))
            {
                ...
                dayFinished = true; breakFound = true;
                return;
            }
        ...
        if (signalArmed)
        {
            if (barOpenUtc > breakCloseUtc)
            {
                ...
                {
                    // === 🧩 Nouveau check : ne pas annuler si une position existe déjà ===
                    bool hasPosition = !IsFlatAccSym_Tolerant();
                    if (hasPosition)
                    {
                        Log("[RETEST EXPIRED SKIPPED] Position détectée sur le même actif et compte → on ne supprime pas l’ordre limit.");
                        retestOrderActive = false;
                        retestLimitOrder = null;
                        signalArmed = false;
                        tradePlaced = true;
