...
public class SBA_LondonBreak_EMA50_6E_Live : Strategy
{
    ...
    protected override void OnRun()
    {
        ...
        // Historique depuis minuit Londres
        ...
        var utcFrom = TimeZoneInfo.ConvertTimeToUtc(lonMid, tzLon);
    ...
    // ====== Ticks → 1m LIVE cache + déclenchement intrabar ======
    private void OnNewLast(Symbol s, Last last)
    {
        ...
        // 2) 🚀 Déclenchement intrabar immédiat (si activé)
        if (EnterIntrabar && boxReady && !breakFound && !tradePlaced)
        {
            var lonNow = TimeZoneInfo.ConvertTimeFromUtc(ts, tzLon);
            var t = lonNow.TimeOfDay;
            ...
            if (t >= entryStart && t <= entryCut && DayMonthAllowed(lonNow))
            {
                string candidate = null;
                if (price > boxHigh + TickSize && SideAllowed("long")) candidate = "long";
                else if (price < boxLow - TickSize && SideAllowed("short")) candidate = "short";

                if (candidate != null)
                {
                    // Entrée aux extrémités
                    double entry = (candidate == "long") ? boxHigh : boxLow;

                    // SL = EMA50 au dernier close connu (si pas prêt → init sur entry)
                    if (!emaInit) { ema = entry; emaInit = true; }
                    double stop = ema;
                    ...
                    if (risk > 0)
                    {
                        ...
                        tpPx = (side == "long") ? entry + TP_R * riskPts : entry - TP_R * riskPts;

                        cutoffParis = ParisAt(new DateTime(lonNow.Year, lonNow.Month, lonNow.Day, MaxTradeEndHour, MaxTradeEndMinute, 0));
                        beArmed = (BE_R >= 0 && BE_R < TP_R);
                        beActivated = false;
                        entryForBE = entryPx;

                        breakFound = true;  // évite re-trigger
                        Log($"[BREAK INTRABAR] {side.ToUpper()} @ {lonNow:HH:mm:ss.fff} LON | Entry={entryPx:F5} SL(EMA50)={stopPx:F5} TP={tpPx:F5}");
                        PlaceInitialOrder();   // ENVOI IMMÉDIAT
                        tradePlaced = true;
                    }
                }
            }
    ...
    private void OnNewBar(object _, HistoryEventArgs e)
    {
        ...
        foreach (var k in aggTF.Values.Where(v => !validatedBuckets.Contains(v.CloseUtc) && barOpenUtc >= v.CloseUtc).OrderBy(v => v.CloseUtc))
        {
            ...
            // Si EnterIntrabar=false, on valide la cassure à la clôture (fallback)
            if (!EnterIntrabar && boxReady && !breakFound)
            {
                var kCloseLon = TimeZoneInfo.ConvertTimeFromUtc(k.CloseUtc, tzLon);
                var entryStart = new TimeSpan(EntryStartHour, EntryStartMinute, 0);
                var entryCut = new TimeSpan(EntryCutoffHour, EntryCutoffMinute, 0);

                if (IsInRange(kCloseLon.TimeOfDay, entryStart, entryCut) && DayMonthAllowed(kCloseLon))
                {
                    double o = k.O, h = k.H, l = k.L, c = k.C;
                    if (m == 1 && live1mAtClose.TryGetValue(k.CloseUtc, out var lv)) { o = lv.O; h = lv.H; l = lv.L; c = lv.C; }

                    string candidate = null;
                    if (c > boxHigh + TickSize && SideAllowed("long")) candidate = "long";
                    else if (c < boxLow - TickSize && SideAllowed("short")) candidate = "short";
                    ...
                    if (candidate != null && WickOk(candidate, o, h, l, c))
                    {
                        ...
                        if (risk > 0)
                        {
                            ...
                            var cutoffLon = new DateTime(kCloseLon.Year, kCloseLon.Month, kCloseLon.Day, MaxTradeEndHour, MaxTradeEndMinute, 0);
                            cutoffParis = ParisAt(cutoffLon);
                            beArmed = (BE_R >= 0 && BE_R < TP_R); beActivated = false; entryForBE = entryPx;

                            breakFound = true;
                            Log($"[BREAK VALID (CLOSE)] {side.ToUpper()} @ {kCloseLon:HH:mm:ss} LON | Entry={entryPx:F5} SL(EMA50)={stopPx:F5} TP={tpPx:F5}");
                            PlaceInitialOrder();
                            tradePlaced = true;
                        }
                    }
        ...
        // [4] Gestion trade (BE + cutoff)
        if (tradePlaced)
        {
            if (beArmed && !beActivated)
            {
                double favorable = (side == "long") ? (bar.High - entryPx) : (entryPx - bar.Low);
                if (favorable >= BE_R * riskPts) beActivated = true;
            }
            if (beActivated)
            {
                bool back = (side == "long") ? (bar.Low <= entryForBE) : (bar.High >= entryForBE);
                if (back)
                {
            ...
            var nowParis = TimeZoneInfo.ConvertTimeFromUtc(barOpenUtc, TimeZoneInfo.FindSystemTimeZoneById("Europe/Paris"));
    ...
    private void PlaceInitialOrder()
    {
        ...
        // Taille
        int stopTicks = Math.Max(1, (int)Math.Round(Math.Abs(entryPx - stopAbs) / TickSize));
    ...
    private void FinalizeBoxIfAny(DateTime lonNow)
    {
    ...
    private void TryBackfillBoxFromHistory()
    {
        ...
        for (int i = 0; i < hd1m.Count; i++)
        {
            ...
            DateTime openUtc = DateTime.SpecifyKind(b.TimeLeft, DateTimeKind.Utc);
            var lonBar = TimeZoneInfo.ConvertTimeFromUtc(openUtc, tzLon);
            if (lonBar.Date != lonDate) continue;
        ...
        if (used > 0)
        {
            ...
            if (nowLon.TimeOfDay < BoxEndExact)
                Log($"[BOX BACKFILL PARTIAL] low={boxLow:F5} high={boxHigh:F5} mid={boxMid:F5} minutes={used} @ {nowLon:HH:mm} LON", StrategyLoggingLevel.Info);
            else
            {
                boxReady = true;
    ...
    private void ResetDay(DateTime nowUtc)
    {
        ...
        emaInit = false; breakFound = false; tradePlaced = false; beActivated = false; beArmed = false;
    }
