...
# =====================================================
CSV_PATH = "GC.csv"  # <-- change si besoin
...
RETEST_MINUTES = 80           # Fenêtre de retest (strictement après la bougie de cassure)
ENABLE_BE = False             # Pas de break-even
BE_AT_R = 2.25                # (ignoré car BE off)

# Filtres sur la bougie de cassure
BODY_OUTSIDE_FRAC_MIN = 0.6  # % du corps hors box min
RANGE_VS_BOX_MIN = 0       # (range / hauteur de box) min
WICK_OUT_MAX_FRAC = 1.0      # % max de mèche côté cassure (vs range de la bougie)
...
BOX_FILTER_TYPE = "min"       # "max" | "min" | "band"
BOX_MAX = 60.0
BOX_MIN = 2.75
BOX_BAND_MIN = 5.0
BOX_BAND_MAX = 20.5
...
def build_daily_setups(ohlc_paris: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    for date_paris, day_paris in ohlc_paris.resample("D"):
        ...
        day5 = df5.loc[day_paris.index.min(): day_paris.index.max()]
        if day5.empty:
            continue

        # Box from NY open window
        ...
        side, bstart_paris, bend_paris, break_px = first_break_close_out(day5, bh, bl)
        if side is None:
            continue

        r = day5.loc[bstart_paris]
