...
# =====================================================
CSV_PATH = "CL10.csv"  # <-- change si besoin
...
TP_R = 2.9                    # Take profit en R
RETEST_MINUTES = 120           # Fenêtre de retest (strictement après la bougie de cassure)
ENABLE_BE = False             # Pas de break-even
BE_AT_R = 2.25                # (ignoré car BE off)

# Filtres sur la bougie de cassure
BODY_OUTSIDE_FRAC_MIN = 0.7  # % du corps hors box min
RANGE_VS_BOX_MIN = 0.1       # (range / hauteur de box) min
...
def build_daily_setups(ohlc_paris: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    for date_paris, day_paris in ohlc_paris.resample("D"):
        ...
        # --- AJOUT : ne pas trader le mercredi (weekday=2) en heure de New York ---
        day_ny = day_paris.tz_convert(EXCHANGE_TZ)
        if day_ny.index[0].weekday() == 2:
            continue
        # -------------------------------------------------------------------------
