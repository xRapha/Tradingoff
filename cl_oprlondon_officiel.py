...
MAX_TRADE_END_LON   = "21:44"  # Trail jusqu'à 14:30 Londres
...
# Entrée limit + filtre retest (copié du fonctionnement MNQ/6B)
RETEST_MINUTES = 240          # fenêtre de retest en minutes après la bougie de cassure
TICK_SIZE      = 0.01       # taille d'un tick (à adapter si besoin par actif)
...
def build_setups(ohlc_paris: pd.DataFrame, tf: str) -> List[Dict[str, Any]]:
    """
    Version modifiée :
    - Entrée LIMIT au lieu de MARKET.
      * long  : limit = close_cassure - 1 tick
      * short : limit = close_cassure + 1 tick
    - Retest possible STRICTEMENT après la bougie de cassure,
      jusqu'à min(bougie cassure + RETEST_MINUTES, ENTRY_CUTOFF).
    - Si le limit n'est jamais touché dans la fenêtre → pas de trade.
    - SL = EMA50 comme avant, TP_R / BE comme avant.
    """
    ...
    for _, day_paris in ohlc_paris.resample("D"):
        ...
        # Première clôture hors box = bougie de cassure
        signal = None
        for ts_lon, row in win_tf.iterrows():
            c = float(row["Close"])
            if c > bh:
                signal = ("long", ts_lon, c)
                break
            if c < bl:
                signal = ("short", ts_lon, c)
                break
        if signal is None:
            continue

        side, break_ts_lon, break_close = signal
        break_ts_paris = break_ts_lon.tz_convert(LOCAL_TZ)

        # SL = EMA50 as-of (inchangé)
        ema_asof = df_tf["ema50"].loc[:break_ts_paris].dropna()
        if ema_asof.empty:
            continue
        stop_price = float(ema_asof.iloc[-1])

        # Prix d'entrée LIMIT = close cassure ± 1 tick
        if side == "long":
            entry_price = float(break_close - TICK_SIZE)
        else:
            entry_price = float(break_close + TICK_SIZE)

        # Risk en points basé sur EMA50 et limit price
        if side == "long":
        ...
        # Wick% sur la bougie de cassure
        try:
            bar = df_tf.loc[break_ts_paris]
        except KeyError:
            cand = df_tf.loc[:break_ts_paris]
            if cand.empty:
                continue
            bar = cand.iloc[-1]
        ...
        # ===== Cutoff global pour la FIN du trade (MAX_TRADE_END_LON) =====
        cut_lon = pd.Timestamp(
            f"{break_ts_paris.tz_convert(SESSION_TZ).date()} {MAX_TRADE_END_LON}",
            tz=SESSION_TZ
        )
        cut_par = cut_lon.tz_convert(LOCAL_TZ)

        # ===== Fenêtre de retest LIMIT après la bougie de cassure =====
        break_end_paris = break_ts_paris + tf_delta

        # cutoff d'ENTRÉE (ENTRY_CUTOFF_LON, ex: 13:00 Londres ≈ 14:00 Paris)
        break_date_lon = break_ts_paris.tz_convert(SESSION_TZ).date()
        entry_cut_lon = pd.Timestamp(f"{break_date_lon} {ENTRY_CUTOFF_LON}", tz=SESSION_TZ)
        entry_cut_par = entry_cut_lon.tz_convert(LOCAL_TZ)

        # fin max du retest = plus tôt entre :
        # - break_end + RETEST_MINUTES
        # - ENTRY_CUTOFF (pas de nouvelle entrée après 14h Paris)
        retest_end_paris = min(
            break_end_paris + pd.Timedelta(minutes=int(RETEST_MINUTES)),
            entry_cut_par
        )

        if retest_end_paris <= break_end_paris:
            # pas de fenêtre de retest exploitable
            continue

        retest_df = df_tf[(df_tf.index > break_end_paris) & (df_tf.index <= retest_end_paris)]
        if retest_df.empty:
            continue

        # Premier toucher du niveau LIMIT
        touch_mask = (retest_df["Low"] <= entry_price) & (retest_df["High"] >= entry_price)
        retest_hits = retest_df.loc[touch_mask]
        if retest_hits.empty:
            # limit jamais retesté -> aucun trade ce jour-là
            continue

        entry_ts_paris = retest_hits.index[0]

        # ===== trail après la bougie d'entrée (même logique qu'avant) =====
        ...
        if first_idx == 10**9:
            ...
            exit_price = float(stop_price)  # SL = EMA50
